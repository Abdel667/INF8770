...
class Algo3Chi2:
    ...
    def chi_squared_distance(self,hist1, hist2):# vectors need to be normalized and need to have same number of bins
        A = self.normalize_histogram(hist1)
        B = self.normalize_histogram(hist2)
        return chisquare(hist1, hist2).statistic
    ...
    def build_set(self, video_parent_dir,image_hist):# builds the set of euclidian distances
        ...
        for video_dir in os.listdir(video_parent_dir):
            for frame in os.listdir(video_dir):
                ...
                distance = self.chi_squared_distance(image_hist, frame_hist)
