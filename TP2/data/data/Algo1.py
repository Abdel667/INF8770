...
class Algo1:
  ...
  def __init__(self,quant , convYUV):
    self.convYUV = convYUV
    self.quant = quant


  # def initCompress(self):
    
  #   # self.baseImages = [self.processImage(image) for image in self.images]

  # TODO Changement de couleur RGB ou YUV if(type(color) == "rgb"))
  def processImage(self, imageLue):
    ...
    title = f"{'YUV' if self.convYUV else 'RGB'}, self.Quantification: {self.quant[0]}/{self.quant[1]}/{self.quant[2]}, Compression: {compression:.2f}, PSNR: {psnr:.2f}, SSIM: {ssim:.2f}"
    py.title(title)
    py.axis('off')
    py.show()
